...
> Evidence snapshot updated 15 September 2026 IST. Canonical repository: `D:\Code\Vantage`. Active branch `redesign-glass`: `origin/redesign-glass` and live `/health.release_sha` are `b0c2b5717ae69e797c8bf84e5b47bf7f3830284f`; the verified local candidate is ahead of that public baseline. `origin/main` remains `db3023a6be02b05417403437ac3eab9f0866962b`, so branch equality is established but the older main-branch release invariant is not. The current local candidate adds only a 320 px Settings/Health layout fix and its regression assertion; it is not deployed. The shipped web UI matches the approved glass redesign — a collapsible navigation rail, a `Ctrl K` command palette, Bricolage Grotesque / Manrope type, and a Settings shell that folds the three configuration screens. The behaviour, contracts, API, compiler, roles, and MCP surface are unchanged by the redesign.
>
...
| Result | Evidence |
| --- | --- |
| 855 distinct cases passed: 680 API/contracts/integration + 160 web + 15 PostgreSQL/Chromium workflows; all coverage gates passed | `D:\Code\Vantage\docs\VERIFICATION.md` |
